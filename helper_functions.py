...
def getQualVecs(data_dir):
    qual_vec_file = data_dir + "/embed_.07_100dim.txt"
    qual_vecs = pd.read_csv(qual_vec_file, sep = " ", index_col = 0, dtype = {0:str})
    qual_repseqs = pd.read_csv(data_dir + "/seqs_.07_embed.fasta", sep = "\t", header = None)
...
def embed_average(otu, qual_vecs):
    print(np.sum([i == j for i,j in zip(otu.columns.values, qual_vecs.index.values)]))
    print(otu.shape[1])
